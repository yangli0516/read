 ...
 */
/**
 * The saveGlossary service saves entity data for glossary entities lem and inf given data of the form
 *
 *   data =
 *   {
 *     cmd: "createLem",
...
if (count($errors) == 0) {
    ...
    case "createLem":
        if (!$tokGID || (!$catID && !$ednID) || !$lemProps) {
            ...
            if (!$catID) { // need to create Glossary catalog
                ...
                if ($catalog->hasError()) {
                    array_push(
                        $errors, "error creating catalog '".
                               $catalog->getDescription()."' - ".
                               $catalog->getErrors(true)
                    );
                } else {
    ...
    case "linkTok":
        if ($inflection) {// add token to passed inflection
            ...
            if ($inflection->hasError()) {
                array_push(
                    $errors, "error updating inflection '".
                    $inflection->getID()."' - ".$inflection->getErrors(true)
                );
            } else {
                addUpdateEntityReturnData(
                    'inf', $inflection->getID(), 
            ...
            if ($lemma->hasError()) {
                array_push(
                    $errors, "error updating lemma '".
                              $lemma->getValue()."' - ".$lemma->getErrors(true)
                );
            } else {
            ...
            if ($lemEntities) {
                foreach ($lemEntities as $entity) {
                    ...
                    if (array_key_exists($hash, $hashLU)) {
                        array_push(
                            $warnings, "lemma ".$lemma->getGlobalID()." has duplicate inflections ".
            ...
            if (array_key_exists($hash, $hashLU)) {
                ...
                if ($tokInflection->hasError()) {
                    array_push(
                        $errors, "error updating inflection '".
                        $tokInflection->getID()."' - ".
                        $tokInflection->getErrors(true)
                    );
                } else {
                    addUpdateEntityReturnData(
                        'inf', $tokInflection->getID(),
                        'entityIDs', $tokInflection->getComponentIDs()
                    );
                }
            } else {
                ...
                if ($tokInflection->hasError()) {
                    array_push(
                        $errors, "error updating inflection '".
                        $tokInflection->getID()."' - ".$tokInflection->getErrors(true)
                    );
                } else {
    ...
    case "removeLink":
        if ($lemma && $linkToEntGID && $linkTypeID) {
            ...
            if ($link) {
                ...
                addUpdateEntityReturnData(
                    'lem', $lemma->getID(), 
                    'relatedEntGIDsByType', $lemma->getRelatedEntitiesByLinkType()
                );
            }
        }
        break;
...
function unlinkInflectedToken($tokGID,$inflection,$lemma)
{
    ...
    if ($tokCmpIndex !== false) {
        ...
        if ($inflection->hasError()) {
            ...
            if ($lemma->hasError()) {
                array_push(
                    $errors, "error unlinking token $tokGID lemma '".
                             $lemma->getValue()."' - ".$lemma->getErrors(true)
                );
            } else {
                addUpdateEntityReturnData('lem', $lemma->getID(), 'entityIDs', $lemma->getComponentIDs());
