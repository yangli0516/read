...
//        this.createPhoneticUI();
  ...
  createValueUI: function () {
    ...
    $('div.valueInputDiv input', this.valueUI).unbind("input").bind("input", function (e) {
      var curInput = $(this).val(),
        btnText = $('.saveDiv', lemmaVE.valueUI).html();
  ...
  createPhoneticUI: function () {
    ...
    //create label
    this.phoneUI.append($('<div class="propDisplayUI">' +
      '<div class="valueLabelDiv propDisplayElement">' + value + '</div>' +
      '</div>'));
    //create input with save button
    this.phoneUI.append($('<div class="propEditUI">' +
      '<div class="valueInputDiv propEditElement"><input class="valueInput" value="' + editValue + '"/></div>' +
      '<button class="saveDiv propEditElement">Save</button>' +
      '</div>'));
    //attach event handlers
    //click to edit
    $('div.valueLabelDiv', this.phoneUI).unbind("click").bind("click", function (e) {
      $('div.edit', lemmaVE.editDiv).removeClass("edit");
      lemmaVE.phoneUI.addClass("edit");
      $('div.valueInputDiv input', this.phoneUI).focus();
      e.stopImmediatePropagation();
      return false;
    });
    $('div.valueInputDiv input', this.phoneUI).unbind("click").bind("click", function (e) {
      e.stopImmediatePropagation();
      return false;
    });
    //blur to cancel
    $('div.valueInputDiv input', this.phoneUI).unbind("blur").bind("blur", function (e) {
      if (!$(e.originalEvent.explicitOriginalTarget).hasClass('saveDiv')) { //all but save button
        //          lemmaVE.phoneUI.removeClass("edit");
      }
    });
    //mark dirty on input
    $('div.valueInputDiv input', this.phoneUI).unbind("input").bind("input", function (e) {
      var curInput = $(this).val(),
        btnText = $('.saveDiv', lemmaVE.phoneUI).html();
      if ($('div.valueLabelDiv', lemmaVE.phoneUI).html() != $(this).val()) {
        if (!$(this).parent().parent().hasClass("dirty")) {
          $(this).parent().parent().addClass("dirty");
        }
      } else if ($(this).parent().parent().hasClass("dirty")) {
      ...
      if (!curInput && btnText == "Save") {
        $('.saveDiv', lemmaVE.phoneUI).html("Delete").css('color', 'red');
      } else if (curInput && btnText != "Save") {
        $('.saveDiv', lemmaVE.phoneUI).html("Save").css('color', 'white');
      }
    });
    //save data
    $('.saveDiv', this.phoneUI).unbind("click").bind("click", function (e) {
      var lemProp = {};
      if ($('.propEditUI', lemmaVE.phoneUI).hasClass('dirty')) {
        val = $('div.valueInputDiv input', lemmaVE.phoneUI).val();
        lemProp["phonetics"] = val;
        $('div.valueLabelDiv', lemmaVE.phoneUI).html(val);
        $('.propEditUI', lemmaVE.phoneUI).removeClass('dirty');
        lemmaVE.saveLemma(lemProp);
      }
      lemmaVE.phoneUI.removeClass("edit");
      e.stopImmediatePropagation();
  ...
  createCompoundAnalysisUI: function () {
    ...
    $('.saveDiv', this.compUI).unbind("click").bind("click", function (e) {
      var lemProp = {},
        compAnalysis, nodeKey, rootKey = null,
        rootNode, compKeys;
  ...
  /**
   * validateCompAnalysis parses the compAnal input ensuring that is 
   * properly formatted and create a structural node definition
   *
   * @param compAnal string defining the compound structure of this lemma
   *
   * @returns {Object} defining the compound tree structure
   */
  ...
  validateCompAnalysis: function (compAnal) {
    ...
    var lemmaVE = this,
      strAnalysis = compAnal,
      value, markup, hasHead, isHead, lemma, lemID,
      subNodes, subNode, compNodes, i, j, k, subKey, key, l = 0,
      nodeLookup = {},
      lemmaIDs,
      reSqBrkOpen = /\[/g,
      reSqBrkClose = /\]/g,
  ...
  createPOSEditUI: function () {
    var lemmaVE = this,
      ...
      listPOS = [{
          ...
          trmID: 674,
          showSub: "showSubAdj",
          showInfl: "showCase showAdjGender showNumber showAdjConj"
        },
        {
          label: "adp.",
          trmID: 661,
        ...
        },
        {
          label: "noun",
          trmID: 664,
          showSub: "showSubNoun showGramGender",
          showInfl: "showGender showCase showNumber"
        },
        {
          label: "num.",
          trmID: 662,
      ...
      listSubNum = [{
          label: "ord.",
          trmID: 683,
          showInfl: "showCase showGender showNumber"
        },
        {
          label: "card.",
          trmID: 682,
    ...
    $('.saveBtnDiv', posEdit).unbind("click").bind("click", function (e) {
      var value = 'POS',
        posVal, posID, posCF = 3,
        lemProps = {},
        sposVal, sposID, sposCF = 3,
        genVal, genID, genCF = 3,
        classCF = 3,
        declCF = 3, // WARNING!! hardcode to unused until Gandhari declination research is finished
  ...
  createInflectionEditUI: function (infID, attestedGID, btnDiv) {
    var lemmaVE = this,
      ...
      spos = this.isLemma && this.entity.spos ? this.entity.spos : null,
      vclass = this.isLemma && this.entity['class'] ? this.entity['class'] : null,
      inflection, sverb = '686',
      gen, num, infcase, per, tense, mood, conj2nd, icf = [3, 3, 3, 3, 3, 3, 3, 3],
      infEdit = this.inflectionEditUI,
      lemmaShowInf = [],
      posSelectedBtns,
      listSubVerb = [{
          label: "Finite",
          trmID: '686',
          showSub: "showSubVerb showNumber showPerson showTense showMood"
        },
        {
          label: "Derivative",
          trmID: '687',
          showSub: "showSubVerb showV2ndConj"
        }
      ],
      listGen = [{
          label: "m.",
  ...
  createAttestedUI: function () {
    var lemmaVE = this,
      i, j, prefix, id, infID, infEntIDs, attestedEntry, nounID = 664,
      adjID = 674,
      pronID = 667,
      verbID = 685,
      ...
      entities = [],
      infMap = {},
      entity, displayUI, infVal, inflection,
  ...
  createTaggingUI: function () {
    var lemmaVE = this,
      i, j, anoTag, tag, annoID, annoIDs, displayUI, annotation,
  ...
  createRelatedUI: function () {
    var lemmaVE = this,
      i, j, prefix, id, entGID, relEntIDs,
