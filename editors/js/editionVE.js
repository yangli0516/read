...
EDITORS.EditionVE.prototype = {
  ...
  init: function () {
    ...
    this.propMgr = new MANAGERS.PropertyManager({
      id: this.id,
  ...
  getUsedTagsList: function () {
    var usedTagIDs = Object.keys(this.dataMgr.tagIDToAnoID), //get the cur tags list from dataMgr
      ...
      items = [],
      i, item, tagID, anoID, label, color, cssSelectors, scope = "#" + this.contentDiv.attr('id');
  ...
  setTagView: function () {
    var endVE = this,
      tagID2Path = this.dataMgr.entTagToPath,
  ...
  getUsedSeqsList: function () {
    var usedSeqIDs = Object.keys(this.seqGIDsByType), //get the cur seqIDs
      colorChoices = {
        "Analysis": "#C2CBCE", //warning!!! term dependency
  ...
  calcSeqList: function () {
    var ednVE = this,
      seqIDs = this.edition.seqIDs,
      i, k, l, entTag, childSeqTag, seqID, seqGID, ednSeqTag,
      seqType, prefix, typeID, sequence, entity, seqIDsByType = {},
      parentTagBySeqTag = {},
      entTagsBySeqTag = {};
  ...
  injectSeqMarkers: function (seqTypeTag, color) {
    var ednVE = this,
      seqGIDs = this.seqGIDsByType[seqTypeTag],
      i, seqGID, label, seqTag,
  ...
  refreshTagMarkers: function () {
    var ednVE = this,
      element,
      checkedItems, cntNewCheckedItems = 0,
      i, item, dropDownContent = '',
      styles = '',
  ...
  refreshSeqMarkers: function (seqIDs) {
    var ednVE = this,
      element, segTypeTag, color, label,
  ...
  afterUpdate: function (entTag) {
    var ednVE = this,
      prefix, entID, entity, $elems, classes, match, txtDivSeqID, lineOrd, $header, physLineID;
  ...
  createStaticToolbar: function () {
    ...
    this.showTagDdBtn.jqxDropDownButton({
      width: 95,
      height: 25
    });
    this.showTagDdBtn.jqxDropDownButton('setContent', '<div class="listDropdownButton">Off</div>');
    ...
    this.showSeqTree.on('checkChange', function (event) {
      var args = event.args,
        ...
        dropDownContent = '',
        i, item = ednVE.showSeqTree.jqxTree('getItem', element),
        segTypeTag = item.id,
        color = item.value,
    ...
    this.showSeqDdBtn.jqxDropDownButton({
      width: 95,
      height: 30
    });
    ...
    $('#' + btnLinkOrdName, this.linkOrdBtnDiv).unbind('click')
      .bind('click', function (e) {
        var ednID = ednVE.edition.id,
          sclIDs = [],
          text = ednVE.dataMgr.getEntity('txt', ednVE.edition.txtID),
          blnIDs = text.blnIDs,
          sclGIDs = ednVE.getSelectionOrdEntityGIDs('scl'),
          index, pos;
    ...
    this.tagTree.on('expand', function (event) {
      var item = ednVE.tagTree.jqxTree('getItem', event.args.element),
        isOldAnchorParentOfItem = false,
        item, itemElement, parentItem;
    ...
    this.tagDdBtn.jqxDropDownButton({
      width: 95,
      height: 25
    });
  ...
  splitToken: function (cbError) {
    var ednVE = this,
      sclEd = this.sclEd,
      startNode, prevNode, headerNode, grdCnt = 500,
      context, refDivSeqID, grpClass = "",
      hdrClass = "",
      sclIDs,
  ...
  createCompound: function (cbError) {
    var ednVE = this,
      ...
      context, refDivSeqID, grpClass = "",
      hdrClass = "",
      sclIDs, bndryVal = sclEd.getCurTokenBoundary(),
      refSclID, newTokID, physLineSeqID, adjPhysLineSeqID, adjPhysLineSeqIDs = [],
      trgTokTag, $tokNodes,
    ...
    if (sclEd && sclEd.syllable && sclEd.syllable.length && bndryVal == 0) {
      ...
      if (sclEd.syllable[0].className.match(/tok\d+/)) {
        ...
        adjPhysLineSeqIDs = $tokNodes.map(function (index, elem) {
          ...
          return [
            [curLineOrdTag, adjPhysLineSeqID]
          ];
        });
      }
  ...
  combineTokens: function (direction, cbError) {
    var ednVE = this,
      sclEd = this.sclEd,
      startNode, prevNode, headerNode, grdCnt = 500,
      sylIsSplit = sclEd.isSplitSyllable(),
      i, context, className,
      grpClass = "",
      hdrClass = "",
      sclIDs, sylOrd, sylNode2, context2, txtDivSeqID,
  ...
  insertFreeTextLine: function (cbError) {
    var ednVE = this,
      sel, range, startNode, prevNode, headerNode, grdCnt = 500,
      context, refDivSeqID, refEntGID, grpClass = "",
      hdrClass = "",
      newLineHTML = "",
      newSclID, newTokGID, newTextDivSeqGID, newPhysLineSeqID, newLineOrd, refLineSeqGID,
      insertdata, refTokID, posInsert = 'after',
      beforeFirst = false;
  ...
  deleteLine: function (cbError) {
    var ednVE = this,
      ...
      deletedata, refTokID, posInsert = 'before',
      beforeFirst = false;
    if (window.getSelection) {
      sel = window.getSelection();
      if (sel.getRangeAt && sel.rangeCount) {
  ...
  getSelectionEntityGIDs: function () {
    var ednVE = this,
      startNode, endNode, nextNode, prevNode, selectNode, startOrd, endOrd, ord,
      grdCnt = 500,
      curGID, lastGID, prefix = this.level2Prefix[this.layoutMgr.getEditionObjectLevel()];
    if (this.autoLinkOrdMode) {
      prefix = 'scl';
    }
    if (window.getSelection) {
  ...
  getSelectionOrdEntityGIDs: function (prefix) {
    var entGID = null,
      selectedGIDs = null,
      regexEntGID = new RegExp(prefix + "\\d+");
  ...
  refreshProperty: function (selectNodeClasses) {
    var ednVE = this,
      entTag, objLevel = ednVE.layoutMgr.getEditionObjectLevel();
    if (objLevel == 'token') {
  ...
  addEventHandlers: function () {
    ...
    function grpGraDblClickHandler(e) {
      var classes = $(this).attr('class'),
        ...
        firstGroup, lastGroup, ord, minOrd = 10000000,
        maxOrd = 0,
        entTag, sandhiSelector = "",
        headernode, segIDs = [],
        i, sclIDs = [],
        sclID, lineord;
    ...
    function keyDownHandler(e) {
      var sclEditor = ednVE.sclEd,
        tcmEditor = ednVE.tcmEd,
        direction, $adjNode, $prevNode, loopLimit = 5,
    ...
    function keyUpHandler(e) {
      var sclEditor = ednVE.sclEd,
        tcmEditor = ednVE.tcmEd,
        startNode, range, sel, selectNode, hint, grdCnt = 500,
    ...
    function sclMouseUpHandler(e) {
      var sel, range, sRange, eRange, isCaretOnly, isReverseSelection, classes,
        ...
        startNode, endNode, tokNode, sclNode, nextNode, prevNode, useTarget = false,
        eTarget,
    ...
    function veScrollHandler(e) {
      var top = this.scrollTop,
        i = 1,
        maxLine = 800,
        grpGra, segTag, lineFraction, imgScrollData;
  ...
  removeFreeTextLineUI: function () {
    ...
    $('.freetextInput', ednVE.contentDiv).each(function (index, elem) {
      var $freeTextInput = $(elem),
        $freeTextDiv = $freeTextInput.parent(),
  ...
  createEditFreeTextLineUI: function ($freeTextDiv) {
    var ednVE = this,
      classes = $freeTextDiv.attr('class'),
      validateData, sel, range, selElem,
      $freeTextInput, validateBtn, errMsgDiv, resultsTable, lostFocusToValidate = false,
      ord = classes.match(/ordL(\d+)/)[1],
      html, freeTextLineSequence, tranlit, //errXoffset = 10, errYoffset=10,
      freeTextLineSeqID = classes.match(/lineseq(\d+)/)[1];
    ednVE.removeFreeTextLineUI();
    if (freeTextLineSeqID) {
      freeTextLineSequence = ednVE.dataMgr.getEntity('seq', freeTextLineSeqID);
  ...
  getImageScrollData: function (segTag, lineFraction) {
    var segID = segTag.substring(3),
      lineSegData = this.scrollDataBySegID[segID],
      imgData = {};
    if (lineSegData) {
  ...
  calcLineScrollDataBySegLookups: function () {
    if (this.physSeq && this.physSeq.entityIDs && this.physSeq.entityIDs.length) {
      var physLineSeqIDs = this.physSeq.entityIDs,
        cnt = physLineSeqIDs.length,
        i, j, segID, lastSegID, prevImgSegID,
        lineSeq, segment, scrollDataIndex,
        blnID, syllable, lineCnt = 0,
        imgSegLineCnt = 0;
  ...
  calcGraLookupTCMNodes: function (prevGraID, graID, nextGraID, sclID, ord) {
    var entities = this.dataMgr.entities,
      prevState, curState, nextState, prevCommon, nextCommon,
  ...
  calcLineGraphemeLookups: function (physLineSeqID) {
    ...
    var entities = this.dataMgr.entities,
      physLineSeq = entities.seq[physLineSeqID],
      isNumber, sclID, sclIDs, graID, graIDs, i, j, prevGraID = null,
      nextGraID,
  ...
  walkTokenContext: function (entGID, context, isLastEnt, footnoteHtml) {
    var entities = this.dataMgr.entities,
      entID = entGID.split(":", 2),
      entType, isCmpTokenSeparator,
      k, ctxt, entIDs, graID, graIDs, isNumber, compound = null,
      entFootnote = null,
      isProperNoun, pos, cmpntGID;
  ...
  renderPhysicalLine: function (physLineSeqID, headerClass, groupClass, grpOrdStart, lineord) {
    ...
    var entities = this.dataMgr.entities,
      physLineSeq = entities.seq[physLineSeqID],
      fnCtx,
      grpOrd = grpOrdStart ? grpOrdStart : 1,
      sclID, sclIDs, i, j, hasNonReconConsnt, hasNonAddedConsnt, isConsnt, typ,
      graLU, grapheme, previousA, previousGraTCMS, graID, graIDs, lineHTML, grpHTML = "",
      inRestore = false,
      prevGraIsVowelCarrier = false,
      plusSign = (this.repType == "diplomatic" ? "+&nbsp" : "+");
    if (physLineSeq && physLineSeq.entityIDs && physLineSeq.entityIDs.length) {
  ...
  renderEdition: function () {
    ...
    var j, cnt = textDivSeqIDs.length,
      nextLineLabel, tcms = 'S',
      ret,
  ...
  renderEditionStructure: function () {
    ...
    function renderWordHTML(entGID, parentDiv) {
      var entities = ednVE.dataMgr.entities,
        tokID, tokIDs, i, j, entID = entGID.substring(4),
        firstT, firstG, lastT, lastG, preTCM, postTCM, isScribalDelete,
        prefix = entGID.substring(0, 3),
        entity = entities[prefix][entID],
...
//                           (!grapheme.txtcrit|| grapheme.txtcrit == "S"))) {
    ...
    function renderStructHTML(seqID, level, parentDiv) {
      var lvl = level + 1,
        seqHtml, proseDiv, i, entGID, prefix, entID,
        sequence = entities['seq'][seqID],
        sectionDiv, wrapperDiv, seqIDs, seqType;
      if (sequence) {
