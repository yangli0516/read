...
class Edition extends Entity
{
    ...
    public function __construct( $arg = null )
    {
        ...
        if (is_numeric($arg) && is_int(-1 + $arg + 1) && $arg > 0) {// this is an ID so need to query the db
            $dbMgr = new DBManager();
            // $dbMgr->query("SELECT * FROM edition WHERE edn_id = $arg".
            //               (isSysAdmin()?"":" AND (".getUserID()."= edn_owner_id or ".
            //               getUserID()." = ANY (\"edn_visibility_ids\"))")." LIMIT 1");
        ...
        if (is_array($arg)) {//initialize from resultRow
            ...
            if (!array_key_exists('edn_id', $arg)) {// must be a new record
                ...
                if (array_key_exists('edn_sequence_ids', $arg)) { 
                  $arg['edn_sequence_ids'] = $this->idsToString($arg['edn_sequence_ids']);
                }
