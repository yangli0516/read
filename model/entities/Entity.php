...
abstract class Entity
{
    ...
    protected function kvPairsStringToArray($kvPairsString)
    {
        ...
        if (is_array($kvPairsString)) { 
          return $kvPairsString;
        }
        return null;
    }
    ...
    protected function arraysOfIdsStringToArray($arrayIdsString)
    {
        ...
        if (is_array($arrayIdsString) && count($arrayIdsString) && is_array($arrayIdsString[0])) {
           return $arrayIdsString;
        }
        return null;
    }
    ...
    protected function arraysOfIdsToString($arrayOfArrayOfIds)
    {
        ...
        if (is_string($arrayOfArrayOfIds)) { 
          return $arrayOfArrayOfIds;
        }
        return null;
    }
    ...
    public function isReadonly()
    {
        if ($this->_visibility_ids && $this->getVisibilityIDs() && in_array(2, $this->getVisibilityIDs()) && !in_array(6, $this->getVisibilityIDs()) 
            ...
            && !in_array($this->getOwnerID(), getUserMembership()))
        ) { // decision to require impersonation for members && !in_array($this->getOwnerID(), getUserMembership()))) {
            return true;
        }
        return false;
    }
        ...
        if ($this->_dirty && count($this->_data)) {
            ...
            if ($this->_id) { // update
                if ((isset($this->_owner_id) && $this->_owner_id != getUserID() && !in_array($this->_owner_id, getUserMembership()) 
                    ...
                    && !isSysAdmin()
                ) {//owner/admin not equal user current user id
